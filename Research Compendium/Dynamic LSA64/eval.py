# batch_eval.py
# -----------------------------------------------------------
# Batch evaluation for LSA64 "cut" dataset.
# - No CLI args: edit CONFIG AREA below.
# - Loads a trained VideoTransformerClassifier checkpoint + meta.
# - Evaluates on a chosen split (val/test/all) by actor.
# - Uniformly samples NUM_FRAMES per video, resizes to FRAME_SIZE.
# - Uses AMP and optional TF32 for speed.
...
# Dataset folder (flat) with files named "XXX_YYY_ZZZ.mp4"
DATA_DIR = r"./datasets/lsa64_raw/all"
...
def sample_frames_decord(path: str, num_frames: int) -> torch.Tensor:
    ...
    frames = vr.get_batch(idxs).asnumpy()  # decord NDArray -> numpy
...
def evaluate():
    ...
    # Load ckpt + meta (explicitly keep pickle behavior)
    ...
    out_dir = Path(OUTPUT_DIR) / f"eval_{EVAL_SPLIT}_{ts}"
    ...
    def flush_batch():
        nonlocal correct_total, cm, per_class_counts
    ...
    for it in pbar:
        ...
        if len(batch_clips) >= INFER_BATCH_SIZE:
            ...
            acc_running = correct_total / max(1, cm.sum())
            pbar.set_postfix({"acc": f"{acc_running*100:.2f}%"})
    ...
    # Save confusion matrix PNG (labels optional — 64 classes fit but small)
    try:
