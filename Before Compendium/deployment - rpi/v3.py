...
def parse_arguments():
    ...
    p.add_argument('--yolo_conf',    type=float, default=0.30, help='Detection confidence threshold')  # Much lower for initial detection
    p.add_argument('--yolo_iou',     type=float, default=0.45, help='NMS IoU threshold')  # Back to original
    p.add_argument('--yolo_imgsz',   type=int,  default=512,   help='(unused: TFLite infers from model), kept for compatibility')

    p.add_argument('--cls_tflite',   type=str, default='models/model.tflite', help='Path to ASL classifier (.tflite)')
    p.add_argument('--cls_threads',  type=int, default=0, help='Classifier TFLite threads; 0=auto')
    p.add_argument('--confidence',   type=float, default=0.8, help='ASL classification accept threshold')  # Lowered default
    ...
    # Performance knobs
    p.add_argument('--det_every', type=int, default=4, help='Run detector every N frames (track in-between)')  # More frequent detection
    p.add_argument('--cls_every', type=int, default=1, help='Run classifier every M frames')  # More frequent classification
    p.add_argument('--smooth_k',  type=int, default=6, help='Smoothing window for prediction averaging')  # Larger smoothing
    p.add_argument('--no_track',  action='store_true', help='Disable tracking (detect every frame)')

    # New preprocessing options
    p.add_argument('--crop_pad',  type=float, default=1, help='Padding factor for hand crop')  # Back to original
    p.add_argument('--min_box_size', type=int, default=10, help='Minimum bounding box size in pixels')  # Much smaller
...
def draw_performance_hud(frame, fps):
    ...
    cpu_percent = psutil.cpu_percent(interval=0.0)  # non-blocking & cheap
...
def main():
    ...
    in_hw = clf.target_hw  # (W,H) for classifier crop resize
