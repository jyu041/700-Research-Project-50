...
def load_meta_for_model(model_path: str, meta_path_override: Optional[str]) -> dict:
    ...
    mp = Path(model_path)
    parent = mp.parent
    # find any *_meta.json in the same folder; pick newest
...
def main():
    ...
    # Resize(short side->FRAME_SIZE, bicubic) -> CenterCrop(FRAME_SIZE) -> Normalize
    ...
    while True:
        ...
        if do_infer:
            # Apply transforms PER FRAME first, then stack to avoid shape-assignment issues
            frames_list: List[torch.Tensor] = []
            for f in frame_buf:
                # f: HxWx3 uint8 -> [3,H,W] float32 in [0,1]
                ft = torch.from_numpy(f).permute(2, 0, 1).float().div(255.0)
                ft = per_frame_transform(ft)   # Resize(short side), CenterCrop, Normalize
                frames_list.append(ft)

            frames_t = torch.stack(frames_list, dim=0)  # [T,3,FRAME_SIZE,FRAME_SIZE]
